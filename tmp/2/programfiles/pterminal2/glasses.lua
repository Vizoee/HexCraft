...
function create(nX, nY, nWidth, nHeight, bStartVisible)
    ...
    local tCursorColor = tHex[colors.black]

    local canvas = peripheral.wrap("back").canvas3d().create()
    local window = {}
    ...
    local function getPlayerRotation()
        local player = peripheral.wrap("back").getMetaByName("Vizoee")
        local yaw = player.yaw
        return {0, 180-yaw, 0}
    end


    function rotateVectorByYaw(vector, yaw)
        yaw = -yaw / 180 * math.pi
        local cosYaw = math.cos(yaw)
        local sinYaw = math.sin(yaw)
    
        local x = vector[1] * cosYaw - vector[3] * sinYaw
        local y = vector[2]
        local z = vector[1] * sinYaw + vector[3] * cosYaw
    
        return {x, y, z}
    end

    function window.updateRotation()
        local rot = getPlayerRotation()
        local pos1 = {0, 4, -4}
        local pos2 = {-2, 4, -4}
        pos1 = rotateVectorByYaw(pos1, rot[2])
        pos2 = rotateVectorByYaw(pos2, rot[2])
        cFrame.setPosition(table.unpack(pos1))
        cFrame.setRotation(table.unpack(rot))
        cFrame.setPosition(table.unpack(pos2))
        canvas.recenter()
    end
    ...
    local function initLines(x, y, width, height, size)
        ...
        -- canvas.clear()
        local pos = {x=0, y=4, z=-4}
        cFrame = canvas.addFrame(pos)
        cFrame.setDepthTested(false)
        local pos2d = {x=0, y=0}
        cbackgroundFrame = cFrame.addGroup(pos2d)---(charWidth*size*width/2)
        cTextFrame = cFrame.addGroup(pos2d)
    ...
    local function redrawLine(n)
        ...
        for i = 1, #tLine[1] do --  and math.mod(os.clock(), 1) < 0.5
            -- if bVisible and n == nCursorY and i == nCursorX and 
            --         math.mod(os.epoch("utc"), 1000) < 500 then
            --     _ = cLine and cLine[1] and cLine[1][i] and cLine[1][i].setText(cursorChar)
            -- else
    ...
    local function updateCursor()
        if bVisible then
            local cLine = cLines[nCursorY]
            _ = cLine and cLine[1] and cLine[1][nCursorX] and cLine[1][nCursorX].setText(cursorChar)
        end
    end
    ...
    function window.setCursorPos(x, y)
        ...
        local oldCursorY = nCursorY
        nCursorY = math.floor(y)
        if bVisible then
            if oldCursorY ~= nCursorY then
                redrawLine(oldCursorY)
            end
            updateCursorPos()
        end
    end
    ...
    function window.reposition(new_x, new_y, new_width, new_height)
